...
| Textured triangle meshes (`draw-mesh`) | no shipped consumer. **egui is NO LONGER the promoting candidate** (verdict 2026-06-14, docs/egui-wandr-feasibility.md): `draw-mesh` (= `SkCanvas::drawVertices`) has **no W3C Canvas2D analog** → it's a GPU-layer verb, out-of-layer for a Canvas2D contract. egui's real backend is `egui_wgpu → wgpu`, so it's redirected to **wasi:webgpu** (wandr implements that host iface; zero canvas changes). No remaining consumer wants `draw-mesh` here. | **DEFERRED INDEFINITELY** — gouraud-textured-triangle fill belongs on the wasi-gfx/wasi:webgpu lane, not this Canvas2D package. If a future consumer ever needs mesh fill *without* the GPU pipeline: additive function (R2), passes R5 as a primitive, shape pre-validated (wasm-tools, 2026-06-12), promotion must spec index validation (out-of-range → trap) |
